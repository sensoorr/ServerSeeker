...
async fn task_wrapper(socket: SocketAddrV4, pool: Database) {
    ...
    match server.simple_ping().await {
        Ok(response) => {
            match serde_json::from_str::<Server>(&response) {
                Ok(server_data) => {
                    // SILENCED SUCCESS LOG TO IMPROVE PERFORMANCE
                    // info!("SUCCESS: Found {} | Version: {} | Players: {}/{}", socket.ip(), server_data.version.name, server_data.players.online, server_data.players.max);
                    
                    if let Err(e) = pool.update_server(server_data, socket).await {
                        error!("Database Error for {}: {}", socket.ip(), e);
                    }
                }
                Err(e) => debug!("Failed to parse response from {}: {}", socket.ip(), e),
            }
        }
        Err(_) => {}
    }
