...
use sqlx::{PgPool, Row}; // Added Row for counting
use std::fs::File;
...
pub async fn country_tracking(pool: PgPool, config: Config) -> anyhow::Result<()> {
    ...
    let count: i64 = sqlx::query("SELECT COUNT(*) FROM countries")
        .fetch_one(&pool)
        .await?
        .get(0);

    if count > 0 {
        info!("Country table already contains {} rows. Skipping update.", count);
    } else {
        info!("Country table is empty. Starting download...");
...
async fn insert_json_to_table(pool: &PgPool) -> anyhow::Result<()> {
    ...
    std::fs::remove_file("ipinfo.json")?; // Clean up JSON after import
    info!("All done!");
    Ok(())
}