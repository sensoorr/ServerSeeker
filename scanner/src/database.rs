...
impl Database {
	...
	pub async fn update_server(&self, server: Server, socket: SocketAddrV4) -> anyhow::Result<()> {
		...
		sqlx::query(
			"INSERT INTO servers (
		address, port, software, version, protocol, icon, description_raw,
        description_formatted, prevents_chat_reports, enforces_secure_chat,
		first_seen, last_seen, online_players, max_players, country, asn) 
		...
		// Player tracking logic removed here to optimize performance.
